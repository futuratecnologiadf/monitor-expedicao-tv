...
st.set_page_config(
    ...
    page_icon="https://cdn-icons-png.flaticon.com/128/1573/1573493.png",
    layout="wide",
...
def fetch_sales_data(loja):
    """Busca dados do banco e retorna dataframes separados por status"""
    try:
        ...
        # Criar dicionário com dataframes separados por status
        statuses = df['status'].unique()
        dfs = {}
        for status in statuses:
            dfs[status] = df[df['status'] == status].drop_duplicates(subset=['pedido'], keep='first') if not df.empty else pd.DataFrame()
        return dfs
    except:
        return {status: pd.DataFrame() for status in statuses}
    ...
    while True:
        ...
        with placeholder.container():
            ...
            for (status_nome, cfg), col_render in zip(config_kanban.items(), cols):
                with col_render:
                    # Usa o dataframe específico do status
                    subset_view = dfs.get(status_nome, pd.DataFrame()).head(10)

                    st.markdown(f'''
                        <div class="status-header" style="border-bottom-color: {cfg['cor']};">
                            <span style="font-size: 1.5rem;">{cfg['icon']}</span>
                            <span>{status_nome}</span>
                            <span style="font-size: 0.8rem; color: gray;">{len(dfs.get(status_nome, pd.DataFrame()))} PEDIDOS</span>
                        </div>
                    ''', unsafe_allow_html=True)

                    if subset_view.empty:
                        st.markdown("<p style='text-align:center; color:#30363d; margin-top:20px;'></p>", unsafe_allow_html=True)
                    else:
